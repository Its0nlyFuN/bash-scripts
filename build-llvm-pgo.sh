...
-DLLVM_ENABLE_SPHINX=OFF -DLLVM_ENABLE_DOXYGEN=OFF \
...
if [[ ! -d llvm-${PKGVER}.src ]] ; then
	wget -qO llvm.tar.xz --show-progress https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0-rc2/llvm-${PKGVER}.src.tar.xz
	tar xf llvm.tar.xz && rm llvm.tar.xz
	cd llvm-${PKGVER}.src/tools
	wget -qO clang.tar.xz --show-progress https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0-rc2/clang-${PKGVER}.src.tar.xz
	tar xf clang.tar.xz && rm clang.tar.xz
	cd ../projects
	wget -qO lld.tar.xz --show-progress https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0-rc2/lld-${PKGVER}.src.tar.xz
	wget -qO compiler-rt.tar.xz --show-progress https://github.com/llvm/llvm-project/releases/download/llvmorg-10.0.0-rc2/compiler-rt-${PKGVER}.src.tar.xz
...
cd $TOPLEV/stage1
ninja clean
cmake -G Ninja "$TOPLEV/llvm-${PKGVER}.src" -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=/usr/bin/clang \
-DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DCMAKE_INSTALL_PREFIX="$TOPLEV/stage1/install" -DLLVM_USE_LINKER=lld ${COMMONFLAGS} || exit 8
echo "---> STAGE 1"
...
ninja clean
cmake -G Ninja "$TOPLEV/llvm-${PKGVER}.src" -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=$CPATH/clang \
